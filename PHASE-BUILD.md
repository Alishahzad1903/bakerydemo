...
- VideoGen's own documentation — indexed by the file in **`videogen-docs/`** — is your sole
  reference for how to talk to VideoGen. You may fetch the documentation pages that index names.
- **Do not** rely on general/external knowledge for VideoGen API details, and do not use any
  VideoGen source other than that documentation.
- If the documentation does not cover a capability you need, **STOP and report the gap** — do
